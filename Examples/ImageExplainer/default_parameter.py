...
def get_default():
    ...
    args.args_trainer.monte_carlo_gradient_estimator = PytorchDistributionUtils.gradientestimator.REBAR # Ordinary training, Variational Traininig, No Variational Training, post hoc...
    ...
    args.args_trainer.reshape_mask_function = utils_reshape.KernelReshape2D
    ...
    args.args_distribution_module.distribution_module = PytorchDistributionUtils.wrappers.REBARBernoulli_STE
    args.args_distribution_module.distribution = Bernoulli
    args.args_distribution_module.distribution_relaxed = PytorchDistributionUtils.distribution.RelaxedBernoulli_thresholded_STE
    args.args_distribution_module.temperature_init = 0.5
    args.args_distribution_module.test_temperature = 0.1
    args.args_distribution_module.scheduler_parameter = PytorchDistributionUtils.wrappers.regular_scheduler
    args.args_distribution_module.antitheis_sampling = False 


    args.args_classification_distribution_module.distribution_module = PytorchDistributionUtils.wrappers.FixedBernoulli
    ...
    args.args_classification_distribution_module.scheduler_parameter = PytorchDistributionUtils.wrappers.regular_scheduler
    ...
    args.args_compiler.optim_classification = partial(Adam, lr=1e-4, weight_decay=1e-3) #Learning rate for classification module
    args.args_compiler.optim_selection = partial(Adam, lr=1e-4, weight_decay=1e-3) # Learning rate for selection module
    args.args_compiler.optim_selection_var = partial(Adam, lr=1e-4, weight_decay=1e-3) # Learning rate for the variationnal selection module used in Variationnal Training
    args.args_compiler.optim_distribution_module = partial(Adam, lr=1e-4, weight_decay=1e-3) # Learning rate for the feature extractor if any
    args.args_compiler.optim_baseline = partial(Adam, lr=1e-4, weight_decay=1e-3) # Learning rate for the baseline network
    args.args_compiler.optim_autoencoder = partial(Adam, lr=1e-4, weight_decay=1e-3)
    args.args_compiler.optim_post_hoc = partial(Adam, lr=1e-4, weight_decay=1e-3)
