...
if __name__ == "__main__":
    ...
    prefix = 'Convv2_autoencoderreconstructionreg_reg01'
    i=0
   #============ No Variationnal ===============
                ...
                for lambda_reg in lambda_reg_list :
                    for epoch_pretrain in nb_epoch_pretrain :
                        ...
                        for noise_function in noise_list :
                            if len(reconstruction_regularization_list) == 1 and reconstruction_regularization_list[0] is None \
                                and len(post_process_regularization_list) == 1 and post_process_regularization_list[0] is None :
                            ...
                            for reconstruction_regularization in reconstruction_regularization_list :
                                for post_process_regularization in post_process_regularization_list :          
                                    ...
                                    if need_autoencoder :
                                        ...
                                        save_interpretation(os.path.join(final_path, "autoencoder"),
                                    ...
                                    classifier_var = ConvClassifierV2(input_size_classifier, mnist.get_category())
                                    ...
                                    save_interpretation(os.path.join(final_path,"results_explain"),sample_list, data, target, suffix = "no_var",
                                         y_hat = torch.exp(pred).detach().cpu().numpy(),
                                         class_names=[str(i) for i in range(10)])
                                        
                                    # print(torch.exp(pred).detach().cpu().numpy())
                                    # print(target)
