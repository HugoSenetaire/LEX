...
class GaussianDataset(ArtificialDataset):
    def __init__(self, mean, cov, covariance_type = 'spherical', dim = 11, nb_sample_train = 20, nb_sample_test = 20, give_index = False, noise_function = None, force_diagonal_impute = False, **kwargs):
        super().__init__(nb_sample_train = nb_sample_train, nb_sample_test = nb_sample_test, give_index = give_index, noise_function=noise_function, **kwargs)
        self.mean = mean
        self.cov = cov
        ...
        if self.covariance_type == 'spherical' :
            ...
            assert self.cov.shape[0] == self.dim
    ...
    def generate_data(self):
        if self.covariance_type == 'spherical' :
            normal_distrib = torch.distributions.normal.Normal(torch.full((self.dim,), fill_value= self.mean),torch.full(size =(self.dim,), fill_value = self.cov))
        elif self.covariance_type == 'diagonal' :
            normal_distrib = torch.distributions.normal.Normal(torch.full((self.dim,), fill_value= self.mean), self.cov)
        elif self.covariance_type == 'full':
            normal_distrib = torch.distributions.multivariate_normal.MultivariateNormal(torch.full((self.dim,), fill_value= self.mean), self.cov)
        else :
...
#     optimal_S_train = torch.cat([torch.ones((nb_sample_train, 2)),torch.zeros((nb_sample_train, 8))], axis=1)
#     optimal_S_test = torch.cat([torch.ones((nb_sample_test, 2)),torch.zeros((nb_sample_test, 8))], axis=1)

#     return X_train, Y_train, optimal_S_train, X_test, Y_test, optimal_S_test

# class XOR(GaussianDataset):
...
#     Y = torch.Bernoulli(prob_test).sample()
#     Y_train,  Y_test = Y[:nb_sample_train], Y[nb_sample_train:]
#     optimal_S_train = torch.cat([torch.ones((nb_sample_train, 4)),torch.zeros((nb_sample_train, 6))], axis=1)
#     optimal_S_test = torch.cat([torch.ones((nb_sample_test, 4)),torch.zeros((nb_sample_test, 6))], axis=1)


#     return X_train, Y_train, optimal_S_train, X_test, Y_test, optimal_S_test

# class OrangeSkin(GaussianDataset):
#     def __init__(self, sigma=1.0, nb_sample_train = 20, nb_sample_test = 20, give_index = False, noise_function = None, **kwargs):
...
class NonLinearAdditiveModel(GaussianDataset):
    def __init__(self, sigma=1.0, nb_sample_train = 20, nb_sample_test = 20, give_index = False, noise_function = None, **kwargs):
        super().__init__(sigma=sigma, nb_sample_train = nb_sample_train, nb_sample_test = nb_sample_test, give_index = give_index, noise_function= noise_function, **kwargs)
        print(f"Given sigma is {self.sigma}")
