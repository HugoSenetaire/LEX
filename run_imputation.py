...
if __name__ == "__main__":
    ...
    imputationMethod_list = [
        ConstantImputationRateReg(), ConstantImputationInsideReg(), ConstantImputationInsideReverseReg(),
        ConstantImputationRateReg(rate=0.1), ConstantImputationInsideReg(rate=0.1), ConstantImputationInsideReverseReg(rate=0.1),
        ConstantImputationRateReg(rate=0.9), ConstantImputationInsideReg(rate = 0.9), ConstantImputationInsideReverseReg(rate=0.9),
                            ]
    # imputationMethod_list = [MaskConstantImputation()]


    path_save = "D:\DTU\TestNewImputationVariation"
   ...
   #============ No Variationnal ===============
    ...
    for dataset_inside in list_dataset:
        folder = os.path.join(path_save,os.path.join("Linear no variationnal", "simplevariation"))
        for lr in lr_list :
            for lambda_reg in lambda_reg_list :
                # for lambda_reg_var in lambda_reg_list :
                    for imputation in imputationMethod_list :
                        ...
                        dataset = LoaderEncapsulation(dataset_inside)             
                        parameter = {
                            "lr":lr,
