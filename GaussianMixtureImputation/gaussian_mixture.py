...
class GaussianMixtureImputation(nn.Module):
  ...
  def __call__(self, data_imputed, sample_b, index = None,):
    """ Using the data and the mask, do the imputation and classification 
        
        Parameters:
        -----------
        data_imputed : torch.Tensor of shape (nb_imputation, batch_size, channels, size_lists...)
    ...
    nb_imputation = data_imputed.shape[0]
    batch_size = data_imputed.shape[1]
    other_dim = data_imputed.shape[2:]
    ...
    data = data_imputed[0].detach().unsqueeze(1).expand(wanted_shape).reshape(wanted_shape_flatten)
    sample = sample_b[0].detach().unsqueeze(1).expand(wanted_shape).reshape(wanted_shape_flatten)
    ...
    dependency = -(data - centers)**2/2/variance - torch.log(variance)/2
    dependency = torch.sum(dependency* sample,axis=-1) + torch.log(weights)
    ...
    index_resampling = torch.distributions.Multinomial(probs = dependency).sample((nb_imputation,)).type(torch.int64)
