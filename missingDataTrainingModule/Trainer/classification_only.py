...
class ordinaryTraining():
    def __init__(self, complete_module, post_hoc = False, post_hoc_guidance = None, argmax_post_hoc = False,):    
        self.complete_module = complete_module
        self.compiled = False
        self.use_cuda = False
    ...
    def _train_step(self, data, target, dataset, index = None, loss_function = NLLLossAugmented(reduction='none'),):
        ...
        log_y_hat, _ = self.complete_module(data, index= index)
...
class trainingWithSelection(ordinaryTraining):
    def __init__(self, complete_module, post_hoc = False, post_hoc_guidance = None, argmax_post_hoc = False,):   
        super().__init__(complete_module, post_hoc, post_hoc_guidance, argmax_post_hoc)
    ...
    def _train_step(self, data, target, dataset, index=None, nb_sample_z_monte_carlo = 1, nb_sample_z_iwae = 1, loss_function = continuous_NLLLoss(reduction = "none") ):
        ...
        log_y_hat, regul_classification, mask, regul_sel, p_z = self.complete_module(data, index= index, nb_sample_z_monte_carlo=nb_sample_z_monte_carlo, nb_sample_z_iwae=nb_sample_z_iwae, )
