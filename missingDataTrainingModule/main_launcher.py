...
def save_parameters(path, complete_args):
    ...
    # for k in inspect.getmembers(complete_args):
    #     if not k[0].startswith('_') and not inspect.ismethod(k[1]) :
    #         with open(os.path.join(complete_path,k[0].replace("args_","")+".txt"), "w") as f:
    #             f.write(str(k[1].to_dic()))
...
def experiment(dataset, loader, complete_args):
    ...
    regularization = get_regularization_method(complete_args.args_selection, complete_args.args_distribution_module)
    ## Sampling :
    distribution_module = get_distribution_module_from_args(complete_args.args_distribution_module)
    classification_distribution_module = get_distribution_module_from_args(complete_args.args_classification_distribution_module)
    ### Imputation :
    imputation = get_imputation_method(complete_args.args_classification, dataset)
    ### Networks :
    classifier, selector, baseline, selector_var, reshape_mask_function = get_networks(complete_args.args_classification, complete_args.args_selection, complete_args.args_trainer, dataset.get_dim_output())
    ### Loss Function :
    loss_function = get_loss_function(complete_args.args_train, dataset.get_dim_output())
    ...
    if complete_args.args_train.post_hoc and complete_args.args_train.post_hoc_guidance is not None :
        print("Training post-hoc guidance")
        post_hoc_classifier =  complete_args.args_train.post_hoc_guidance(complete_args.args_selection.input_size_selector, dataset.get_dim_output())
        ...
        if complete_args.args_train.use_cuda:
            trainer.cuda()

        optim_post_hoc, scheduler_post_hoc = get_optim(post_hoc_guidance, complete_args.args_compiler.optim_post_hoc, complete_args.args_compiler.scheduler_post_hoc)
        optim_post_hoc = complete_args.args_compiler.optim_post_hoc(post_hoc_guidance.parameters())
        trainer.compile(optim_classification=optim_post_hoc, scheduler_classification = scheduler_post_hoc)

        total_dic_train = {}
        total_dic_test = {}
        for epoch in range(complete_args.args_train.nb_epoch_post_hoc):
            ...
            test_this_epoch = complete_args.args_trainer.save_epoch_function(epoch, complete_args.args_train.nb_epoch_post_hoc)
            if test_this_epoch :
    ...
    if (complete_args.args_trainer.complete_trainer is EVAL_X) or (complete_args.args_trainer.complete_trainer is REALX) :

            classification_module = ClassificationModule(classifier, imputation)

            if  complete_args.args_trainer.complete_trainer is REALX and complete_args.args_train.nb_epoch_pretrain > 0 :
                post_hoc_guidance_eval_x = None
                post_hoc_eval_x = False
            else :
                post_hoc_guidance_eval_x = post_hoc_guidance 
                post_hoc_eval_x = complete_args.args_train.post_hoc
            ...
            trainer_ordinary = EVAL_X(classification_module, 
                            ...
                            argmax_post_hoc = complete_args.args_train.argmax_post_hoc,
                            )
            if complete_args.args_train.use_cuda:
                trainer_ordinary.cuda()

            optim_classification, scheduler_classification = get_optim(classification_module, complete_args.args_compiler.optim_classification, complete_args.args_compiler.scheduler_classification)
            trainer_ordinary.compile(optim_classification=optim_classification, scheduler_classification = scheduler_classification,)
            
            if complete_args.args_trainer.complete_trainer is EVAL_X :
                nb_epoch = complete_args.args_train.nb_epoch
            else :
                nb_epoch = complete_args.args_train.nb_epoch_pretrain
            total_dic_train = {}
            total_dic_test = {}
            for epoch in range(nb_epoch):
                dic_train = trainer_ordinary.train_epoch(epoch, loader, nb_sample_z_monte_carlo = complete_args.args_train.nb_sample_z_train_monte_carlo, loss_function = loss_function, save_dic = True, verbose=True)
                total_dic_train = fill_dic(total_dic_train, dic_train)
                test_this_epoch = complete_args.args_trainer.save_epoch_function(epoch, nb_epoch)
                if test_this_epoch :
                    dic_test = trainer_ordinary.test(epoch, loader, liste_mc = complete_args.args_test.liste_mc)
                    total_dic_test = fill_dic(total_dic_test, dic_test)
        ...
        if complete_args.args_trainer.complete_trainer is ordinaryTraining or complete_args.args_trainer.complete_trainer is trueSelectionTraining :
            nb_epoch = int(complete_args.args_train.nb_epoch)
            post_hoc_guidance_ordinary = post_hoc_guidance
            post_hoc_ordinary = complete_args.args_train.post_hoc
        ...
        if complete_args.args_trainer.complete_trainer is trueSelectionTraining :
        ...
        optim_classification, scheduler_classification = get_optim(vanilla_classification_module, complete_args.args_compiler.optim_classification, complete_args.args_compiler.scheduler_classification)
        trainer_ordinary.compile(optim_classification=optim_classification, scheduler_classification = scheduler_classification,)
    ...
    selection_module = SelectionModule(selector,
                    activation=complete_args.args_selection.activation,
                    regularization=regularization,
                    )

    if complete_args.args_train.nb_epoch_pretrain_selector > 0 :
        selection_trainer = selectionTraining(selection_module, complete_args.args_train.use_regularization_pretrain_selector)
        optim_selection, scheduler_selection = get_optim(selection_module, complete_args.args_compiler.optim_selection, complete_args.args_compiler.scheduler_selection)
        selection_trainer.compile(optim_selection=optim_selection, scheduler_selection = scheduler_selection,)
        nb_epoch = complete_args.args_train.nb_epoch_pretrain_selector
    ...
    trainer = complete_args.args_trainer.complete_trainer(
        classification_module,
        selection_module,
        monte_carlo_gradient_estimator = complete_args.args_trainer.monte_carlo_gradient_estimator,
        baseline = baseline,
        distribution_module = distribution_module,
        classification_distribution_module = classification_distribution_module,
        reshape_mask_function = reshape_mask_function,
        fix_classifier_parameters = complete_args.args_train.fix_classifier_parameters,
        fix_selector_parameters = complete_args.args_train.fix_selector_parameters,
        post_hoc_guidance = post_hoc_guidance,
        post_hoc = complete_args.args_train.post_hoc,
        argmax_post_hoc = complete_args.args_train.argmax_post_hoc,
        )
    ...
    optim_classification, scheduler_classification = get_optim(classification_module, complete_args.args_compiler.optim_classification, complete_args.args_compiler.scheduler_classification) 
    optim_selection, scheduler_selection = get_optim(selection_module, complete_args.args_compiler.optim_selection, complete_args.args_compiler.scheduler_selection)
    optim_baseline, scheduler_baseline = get_optim(baseline, complete_args.args_compiler.optim_baseline, complete_args.args_compiler.scheduler_baseline)
    optim_distribution_module, scheduler_distribution_module = get_optim(distribution_module, complete_args.args_compiler.optim_distribution_module, complete_args.args_compiler.scheduler_distribution_module)
...
###### Main module training :
    ...
    total_dic_train = {}
    total_dic_test = {}
    for epoch in range(complete_args.args_train.nb_epoch):
        dic_train = get_training_method(trainer, complete_args.args_train, trainer_ordinary)(
            epoch, 
            loader,
            save_dic = True,
            nb_sample_z_monte_carlo = complete_args.args_train.nb_sample_z_train_monte_carlo,
            nb_sample_z_iwae = complete_args.args_train.nb_sample_z_train_IWAE,
            loss_function = loss_function,
            verbose = ((epoch+1) % complete_args.args_train.print_every == 0),
        )
        total_dic_train = fill_dic(total_dic_train, dic_train)

        test_this_epoch = complete_args.args_trainer.save_epoch_function(epoch, complete_args.args_train.nb_epoch)
        if test_this_epoch :
            dic_test = trainer.test(epoch, loader, liste_mc = complete_args.args_test.liste_mc)
            if complete_args.args_output.save_weights :
